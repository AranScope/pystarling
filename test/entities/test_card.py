...
def test_get_card(requests_mock):
    ...
    with open("test/responses/v1-get-card.json") as f:
        expected_response = json.load(f)

        requests_mock.get("{}/api/v1/cards".format(mock_url),
                          headers=default_headers(mock_access_token),
                          request_headers=expect_authorization_header(mock_access_token),
                          json=expected_response)

        mocked_response = mock_object.get_card(access_token=mock_access_token)
